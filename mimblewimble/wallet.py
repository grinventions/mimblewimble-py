...
class Wallet:
    def __init__(
        ...
        bip39_passphrase_protected=False,
    ):
        self.encrypted_seed = encrypted_seed
        self.salt = salt
        self.nonce = nonce
        self.master_seed = master_seed  # 32 bytes (legacy) or 64 bytes (BIP39)
        self.mnemonic_phrase = mnemonic_phrase  # stored when using BIP39 mode
        self.bip39_passphrase_protected = bip39_passphrase_protected
    ...
    def _getKeychain(self):
        """Get the KeyChain for this wallet, using BIP39 or legacy mode.

        Returns:
            KeyChain instance derived from the wallet's master seed.
        """
        if self.master_seed is None:
            raise Exception("The wallet is shielded")
        return KeyChain.fromSeed(self.master_seed)
    ...
    def unshieldWallet(self, passphrase: str, salt=None, nonce=None):
        ...
        key = pbkdf2_hmac("sha512", bytes(passphrase, "utf-8"), self.salt, 100)
        cipher = ChaCha20_Poly1305.new(key=key[0:32], nonce=self.nonce)
        # ChaCha20-Poly1305 tag is always 16 bytes, appended after ciphertext
        ciphertext = self.encrypted_seed[:-16]
        tag = self.encrypted_seed[-16:]
        plaintext = cipher.decrypt_and_verify(ciphertext, tag)
        self.master_seed = plaintext
        del plaintext

    def getSeedPhrase(self):
        """Get the mnemonic seed phrase for this wallet.

        In BIP39 passphrase-protected mode, returns the stored mnemonic phrase.
        In legacy mode, derives the mnemonic from the raw entropy.
        """
        if self.mnemonic_phrase is not None:
    ...
    def createCoinbase(self, amount, path="m/0/1/0"):
        ...
        blinding_factor = keychain.derivePrivateKeyAmount(path, amount)
        commitment = p.commit(amount, blinding_factor)

        # bulletproof
        rangeproof = keychain.generateRangeProof(
            path, amount, commitment, blinding_factor, EBulletproofType.ENHANCED
        )
        ...
        transaction_output = TransactionOutput(
            EOutputFeatures.COINBASE_OUTPUT, commitment, rangeproof
        )
        ...
        message = blake2b(
            EKernelFeatures.COINBASE_KERNEL.to_bytes(1, "big"), digest_size=32
        ).digest()
    ...
    def createBlindedOutput(
        ...
        # bulletproof
        rangeproof = keychain.generateRangeProof(
            path, amount, commitment, blinding_factor, bulletproof_type
        )

        # build the output
        transaction_output = TransactionOutput(
            EOutputFeatures.DEFAULT, commitment, rangeproof
        )
    ...
    def receive(
        self, send_slate: Slate, path="m/0/1/0", wallet_tx_id=None, testnet=False
    ) -> Slate:
        ...
        receive_slate, secret_key, secret_nonce = slate_builder.addReceiverData(
            send_slate, output, testnet=testnet
        )
    ...
    def invoice(
        self, amount: int, wallet_tx_id=None, path: str = "m/0/1/0", block_height=0
    ) -> Tuple[Slate, SecretKey, SecretKey]:
        output = self.createBlindedOutput(
            amount, EBulletproofType.ENHANCED, path=path, wallet_tx_id=wallet_tx_id
        )
        slate_builder = InvoiceSlateBuilder(self.master_seed)
        slate, secret_key, secret_nonce = slate_builder.build(
            amount, output, block_height=block_height, slate_version=0
        )
    ...
    def pay(
        ...
        slate, secret_key, secret_nonce = slate_builder.build(
            invoice_slate, fee, inputs, change_outputs
        )
        return slate, secret_key, secret_nonce, change_outputs

    def finalize(
        self, receive_slate: Slate, path="m/0/1/0", wallet_tx_id=None, testnet=False
    ):
        ...
        finalized_slate = slate_builder.finalize(receive_slate, testnet=testnet)

        if check_payment_proof:
            sender_address_bytes = keychain.deriveED25519PublicKey(path)
            slate_builder.verifyPaymentProof(finalized_slate, sender_address_bytes)
    ...
    @classmethod
    def initialize(self, bip39_passphrase: str = None):
        """Create a new wallet with random entropy.

        Args:
            bip39_passphrase: Optional BIP39 passphrase for password-protected
                seed derivation. When provided, the wallet uses BIP39
                PBKDF2 derivation (mnemonic + passphrase → seed), producing
                a different wallet than the same mnemonic without a passphrase.
        ...
        if bip39_passphrase is not None:
            M = Mnemonic()
            mnemonic_phrase = M.mnemonicFromEntropy(entropy)
            bip39_seed = M.mnemonicToSeed(mnemonic_phrase, bip39_passphrase)
            return Wallet(
                master_seed=bip39_seed,
                mnemonic_phrase=mnemonic_phrase,
                bip39_passphrase_protected=True,
            )
        return Wallet(master_seed=entropy)

    @classmethod
    def fromSeedPhrase(self, phrase: str, bip39_passphrase: str = None):
        """Restore a wallet from a mnemonic seed phrase.

        Args:
            phrase: BIP39 mnemonic phrase (space-separated words).
            bip39_passphrase: Optional BIP39 passphrase. When provided,
                uses BIP39 PBKDF2 derivation to produce a password-protected
                wallet. The same mnemonic with different passphrases yields
                entirely different wallets.

        Returns:
            A restored Wallet instance.
        """
        M = Mnemonic()
        if bip39_passphrase is not None:
            bip39_seed = M.mnemonicToSeed(phrase, bip39_passphrase)
            return Wallet(
                master_seed=bip39_seed,
...
class PersistentWallet:
    def __init__(
        self, wallet: Wallet, storage: WalletStorage, node=None, birthday_height=0
    ):
        self._wallet = wallet
