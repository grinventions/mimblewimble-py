...
class AggSig:
    ...
    def generateSecureNonce(self):
        seed = os.urandom(32)
        nonce = secp256k1_aggsig_export_secnonce_single(self.ctx, seed)
        return SecretKey(nonce)

    def buildSignature(
        self, secret_key: SecretKey, commitment: Commitment, message: bytes
    ):
        seed = os.urandom(32)
        ...
        pubkey = secp256k1_pedersen_commitment_to_pubkey(self.ctx, parsed_commitment)
    ...
    def calculatePartialSignature(
        ...
        sumPubKeys: PublicKey,
        sumPubNonces: PublicKey,
        message: bytes,
    ):
        seed = os.urandom(32)
        secp256k1_context_randomize(self.ctx, seed)
        pubKeyForE = secp256k1_ec_pubkey_parse(self.ctx, sumPubKeys.getBytes())
        pubNoncesForE = secp256k1_ec_pubkey_parse(self.ctx, sumPubNonces.getBytes())
        signature = secp256k1_aggsig_sign_single(
            self.ctx,
            message,
            secretKey.getBytes(),
            secretNonce.getBytes(),
            None,
            pubNoncesForE,
            pubNoncesForE,
            pubKeyForE,
    ...
    def verifyPartialSignature(
        ...
        pubkey = secp256k1_ec_pubkey_parse(self.ctx, publicKey.getBytes())
        sumPubKey = secp256k1_ec_pubkey_parse(self.ctx, sumPubKeys.getBytes())
        sumNoncesPubKey = secp256k1_ec_pubkey_parse(self.ctx, sumPubNonces.getBytes())
        is_valid = secp256k1_aggsig_verify_single(
            self.ctx, signature, message, sumNoncesPubKey, pubkey, sumPubKey, None, True
        )
        return is_valid

    def aggregateSignatures(self, signatures, sumPubNonces: PublicKey):
        pubNonces = secp256k1_ec_pubkey_parse(self.ctx, sumPubNonces.getBytes())
    ...
    def verifyAggregateSignature(
        ...
        parsedPubKey = secp256k1_ec_pubkey_parse(self.ctx, publicKey.getBytes())
        is_valid = secp256k1_aggsig_verify_single(
            self.ctx,
