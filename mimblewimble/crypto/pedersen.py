...
class Pedersen:
    ...
    def commit(self, value: int, blindingFactor: BlindingFactor):
        ...
        serialized = secp256k1_pedersen_commitment_serialize(self.ctx, commitment)
    ...
    def blindSum(self, positive: List[Commitment], negative: List[Commitment]):
        ...
        summed = secp256k1_pedersen_blind_sum(self.ctx, blinding_factors, len(positive))
    ...
    def toCommitment(self, public: PublicKey):
        pk = secp256k1_ec_pubkey_parse(self.ctx, public.getBytes())
        commitment = secp256k1_pubkey_to_pedersen_commitment(self.ctx, pk)
        serialized = secp256k1_pedersen_commitment_serialize(self.ctx, commitment)
        return Commitment(serialized)
    ...
    def convertCommitments(self, ctx, commitments: List[Commitment]):
        ...
        for commitment in commitments:
            parsed = secp256k1_pedersen_commitment_parse(ctx, commitment.getBytes())
