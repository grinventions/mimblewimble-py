...
- Node persistence adapters and related utilities are documented in [scripts/README.md](scripts/README.md).
...
### BIP39 Password-Protected Seed (Passphrase)

The wallet supports [BIP39](https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki) passphrase protection, which adds an optional password (sometimes called a "25th word") to the mnemonic recovery phrase. This provides an extra layer of security and enables **plausible deniability** — the same mnemonic phrase with different passphrases will derive entirely different wallets.

#### How it works

1. A standard 24-word mnemonic is generated from random entropy
2. The mnemonic and an optional passphrase are fed into **PBKDF2-HMAC-SHA512** (2048 iterations) with the salt `"mnemonic" + passphrase`
3. This produces a 64-byte BIP39 seed
4. The BIP39 seed is then passed through **HMAC-SHA512** with the Grin-specific key `"IamVoldemort"` to derive the master key
5. All wallet keys (private keys, ED25519, slatepack addresses) are derived from that master key via BIP32

Using no passphrase (empty string) is equivalent to standard mnemonic behavior, so existing wallets are unaffected.

#### Creating a wallet with BIP39 passphrase

```python
from mimblewimble.wallet import Wallet

# create a wallet with passphrase protection
w = Wallet.initialize(bip39_passphrase='my secret passphrase')

# the recovery phrase alone is NOT enough to restore this wallet
# you must also remember the passphrase
path = 'm/0/1/0'
slatepack_address = w.getSlatepackAddress(path=path)
recovery_phrase = w.getSeedPhrase()
...
print(recovery_phrase)
print()
print('Remember: you need BOTH the recovery phrase AND the passphrase to restore this wallet!')
```

#### Restoring a wallet with BIP39 passphrase
...
# restore with the same passphrase used during creation
w = Wallet.fromSeedPhrase(recovery_phrase, bip39_passphrase='my secret passphrase')
...
The same mnemonic with different passphrases produces completely different wallets. This allows you to maintain a "decoy" wallet with a small balance (using an empty or simple passphrase) alongside your real wallet (using a strong passphrase). Under duress, you can reveal the decoy passphrase without exposing the real wallet.
...
# no passphrase — standard wallet (could be your decoy)
w_decoy = Wallet.fromSeedPhrase(phrase)

# with passphrase — completely different wallet
w_real = Wallet.fromSeedPhrase(phrase, bip39_passphrase='correct horse battery staple')

path = 'm/0/1/0'
print('Decoy address:', w_decoy.getSlatepackAddress(path=path))
print('Real address: ', w_real.getSlatepackAddress(path=path))

# These are two entirely different wallets derived from the same mnemonic
```

#### Using KeyChain directly

You can also use the lower-level `KeyChain.fromMnemonic()` method directly for key derivation:
...
# derive keychain without passphrase
kc_default = KeyChain.fromMnemonic(mnemonic)

# derive keychain with passphrase
kc_protected = KeyChain.fromMnemonic(mnemonic, passphrase='my secret')

# these produce different master keys and different slatepack addresses
print('Default address:  ', kc_default.getSlatepackAddress())
print('Protected address:', kc_protected.getSlatepackAddress())
```
