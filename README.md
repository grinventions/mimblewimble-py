...
wallet_alice = Wallet.initialize()
wallet_bob = Wallet.initialize()

# Alice makes 60 coins coinbase output
alice_path = 'm/0/1/0'
coinbase_amount = 60000000000
kernel, output = wallet_alice.createCoinbase(
...
fee_base = 1
block_height = 99999
send_slate = wallet_alice.send(
    [output], num_change_outputs, amount, fee_base, block_height, path=alice_path)

# TODO build the receive slate using Bob's wallet
# TODO finalize the receive slate using Alice's wallet
```
