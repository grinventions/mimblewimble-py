...
def test_rsr_flow_slatepacks():
    ...
    kernel, output = alice_wallet.createCoinbase(coinbase_amount, path=alice_path)
    ...
    invoice_slate, bob_secret_key, bob_secret_nonce = bob_wallet.invoice(
        amount, path=bob_path, block_height=block_height
    )
    invoice_slate_payload = invoice_slate.serialize()

    # prepare R1 invoice slatepack
    version = SlatepackVersion(0, 0)
    metadata = SlatepackMetadata(sender=SlatepackAddress.fromBech32(alice_address))
    ...
    # Alice receives it
    received_slatepack_R1 = alice_wallet.decryptSlatepack(slatepack_R1, path=alice_path)
    ...
    pay_slate, alice_secret_key, alice_secret_nonce, change_outputs = alice_wallet.pay(
        received_invoice_slate, [output], num_change_outputs, fee_base, alice_path
    )
    ...
    pay_slatepack_message.encryptPayload([SlatepackAddress.fromBech32(bob_address)])
    ...
    finalized_slate = bob_wallet.finalize(received_pay_slate, path=bob_path)
...
def test_rsr_flow_slatepacks_persistent():
    ...
    r1_slatepack_message = bob_wallet.invoice(
        30000000000, path=bob_path, receiver=alice_wallet.getSlatepackAddress()
    )
    ...
    with pytest.raises(Exception, match="Insufficient funds"):
        alice_wallet.pay(r1_slatepack_message, path=alice_path, fee_base=fee_base)

    # Alice needs some coinbase to pay Bob's invoice
    coinbase_amount = 60000000000
    coinbase_transaction = alice_wallet.createCoinbase(coinbase_amount, path=alice_path)
    ...
    slate_fee = (
        bob_wallet.decryptSlatepack(r2_slatepack_message).getSlate().getFee().getFee()
    )
    ...
    finalized_slate = bob_wallet.finalize(r2_slatepack_message, path=bob_path)
