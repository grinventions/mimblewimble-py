...
def test_reflexivity():
    ...
    w_restored = Wallet.fromEncryptedSeedDict(seed, passphrase=random_password)
...
def test_invalid_password():
    ...
    try:
        Wallet.fromEncryptedSeedDict(seed, passphrase=invalid_password)
    except Exception as e:
        assert str(e) == "MAC check failed"


def test_restore_from_encrypted_seed():
    w = Wallet.fromEncryptedSeedDict(seed, passphrase=password)
...
def test_grin_plusplus():
    ...
    seed = bytes.fromhex(
        "f9a0e73d3cd533368f75ff63cbd97b2100beffbc339cdfa5c203c1a022d9cf11"
    )
    ...
    assert (
        slatepack_address
        == "grin1q6qnz4y7chpmmjeazw59tcdhd5tejx00m96xdy5jkv30t4v6fnxqv9kwer"
    )
