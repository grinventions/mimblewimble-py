...
def test_srs_flow_slatepacks():
    ...
    send_slate, alice_secret_key, alice_secret_nonce, change_outputs = (
        alice_wallet.send(
            [output],
            num_change_outputs,
            amount,
    ...
    send_slatepack_message.encryptPayload([SlatepackAddress.fromBech32(bob_address)])
    slatepack_S1 = send_slatepack_message.pack()
    assert send_slatepack_message.is_encrypted()
    # now Alice sends slatepack_S1 to Bob

    # Bob receives it
    received_slatepack_S1 = bob_wallet.decryptSlatepack(slatepack_S1, path=bob_path)
    ...
    receive_slate, bob_secret_key, bob_secret_nonce = bob_wallet.receive(
        received_send_slate
    )
    ...
    version = SlatepackVersion(0, 0)
    metadata = SlatepackMetadata(sender=SlatepackAddress.fromBech32(bob_address))
    emode = EMode.PLAINTEXT
    ...
    finalized_slate = alice_wallet.finalize(received_receive_slate, path=alice_path)
...
def test_srs_flow_slatepacks_persistent():
    ...
    with pytest.raises(Exception, match="Insufficient funds"):
        alice_wallet.send(
            30000000000, bob_wallet.getSlatepackAddress(), fee_base=fee_base
        )
    ...
    slate_fee = (
        bob_wallet.decryptSlatepack(s1_slatepack_message).getSlate().getFee().getFee()
    )
    ...
    s2_slatepack_message = bob_wallet.receive(
        s1_slatepack_message_received, path=bob_path
    )
